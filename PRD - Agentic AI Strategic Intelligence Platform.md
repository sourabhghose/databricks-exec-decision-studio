...
**Version:** 1.1 — Updated  
**Date:** April 2026 (revised 2026-04-06)  
...
The Scenario Simulator (accessible via AI Assistant → Simulation) delivers the core simulation loop:

- **15 Alinta-specific quick-start templates** covering: Loy Yang B retirement timing, Yandin Stage 2 FID, Retail Transformation pacing, Capital Structure, WEM Capacity Mechanism, Battery Storage investment, Newman Gas life extension, Retail Pricing Strategy, M&A Distributed Energy, Carbon/Net Zero, Hydrogen feasibility, Digital & AI Transformation, FY2027 Dividend Policy, and Cybersecurity OT Uplift. Users can also enter any custom decision text.
- **Bear / Base / Bull scenario cards** generated by Claude Sonnet 4.6, grounded in live Alinta KPI, financial, and risk data from Unity Catalog. Each card shows: probability, headline outcome, 3–4 key assumptions, 3–5 KPI impact rows (baseline → projected with colour-coded delta badges), risk factors, and strategic rationale.
- **AI recommendation** with explicit Bear/Bull/Base selection criteria — the LLM evaluates open Critical/High risks and above-target KPIs to choose the recommended scenario, not defaulting to Base.
- **Drill-down narrative** — board selects a scenario branch; a second SSE stream generates Implementation Roadmap (5 steps with owners/timelines), Risk Mitigations, 90-Day Board Actions, and Key Metrics to track.
- **Save to Decision Register** — persists the selected scenario outcome to `eds_actions.decision_register` for audit trail.
- **SSE streaming architecture** — tokens stream progressively; backend emits a `result` event with the complete assembled JSON for reliable parsing independent of HTTP/2 chunk boundaries.
...
### 11.0 Build Status (as of 2026-04-06)

The following capabilities have been built and are live at https://exec-decision-studio-7474646159107961.aws.databricksapps.com:

| Capability | Status | Notes |
|------------|--------|-------|
| FastAPI + React SPA on Databricks Apps | ✅ Live | Uvicorn, SSE streaming, static asset serving |
| Unity Catalog data layer (7 schemas) | ✅ Live | `eds_raw`, `eds_processed`, `eds_synthetic`, `eds_actions`, `eds_audit`, `eds_evaluation`, `eds_vectors` |
| 22-document synthetic corpus | ✅ Live | Tier-classified, embedded, indexed in Vector Search |
| RAG pipeline + Vector Search | ✅ Live | `databricks-gte-large-en` embeddings, FAISS-based VS index |
| Strategic Chat (SSE RAG streaming) | ✅ Live | 32 C-level sample questions, inline Recharts charts, markdown tables, PDF/PPTX export |
| Executive Overview with drill-down | ✅ Live | 4 hero cards, 6 KPI cards, charts; right-side drawer with AI analysis |
| KPI Dashboard | ✅ Live | Time-series from `eds_synthetic.kpi_timeseries` |
| Risk Register | ✅ Live | From `eds_synthetic.risk_register` |
| Decision Register | ✅ Live | From `eds_actions.decision_register`; save from Simulator |
| Action Items | ✅ Live | From `eds_actions.action_items` |
| Document Library | ✅ Live | Filter panel, classification badges, click-to-expand modal, document upload |
| AI Briefing (SSE streaming) | ✅ Live | 4 briefing types, markdown tables, PDF/PPTX/MD export |
| Market Intelligence | ✅ Live | GDELT news, Yahoo Finance ASX stocks, CER carbon prices |
| Audit Log | ✅ Live | From `eds_audit.agent_interactions` |
| **Scenario Simulator** | ✅ Live | Bear/Base/Bull, 15 quick-start templates, drill-down, save to decision register |
| SP permissions automation | ✅ Live | `05_grant_app_permissions.py` discovers SP via SCIM, grants all required UC/warehouse permissions |
| DABs bundle + deploy.sh | ✅ Live | `scripts/deploy.sh` canonical 4-step deploy workflow |
| AI Assistant dropdown navigation | ✅ Live | Strategic Chat, AI Briefing, Simulation grouped under Bot icon dropdown |

**Not yet built:**
- Supervisor Agent wired to chat (RAG calls LLM directly)
- Voice interface
- M&A deal room
